...
/**
  ...
  * @retval None
  */
void PCA_CountTimerInit(PCA_TypeDef* PCAx, \
	                      PCA_CountTimerInitTypeDef* PCA_CountTimerInitInitStruct)
{
	int tmpreg = 0;
...
/**
  ...
  * @retval None
  */
void PCA_CaptureITCmd(PCA_TypeDef* PCAx,\
	                    PCA_ICInitTypeDef* PCA_ICInitStruct,\
...
/**
  ...
  * @param  PCAx: Set the PCA peripheral.
  * @param  channel: select pca channel.
  * @param  value:  set the value of the pca channel.   
  * @retval None
  */
void PCA_SetSoftCompare(PCA_TypeDef* PCAx,\
	                      PCA_Channel channel,\
                        uint16_t value)
{
	/*Check the parameters */
...
/** @brief  enable the PCA capture interrupt function
  ...
  * @retval None
  */
void PCA_ClearSoftCompare(PCA_TypeDef* PCAx,\
	                        PCA_Channel channel,\
                          uint16_t value)
{
	/*Check the parameters */
...
/** @brief  config high speed output function
  ...
  * @param  channel: select pca channel.
  * @param  value: set the value of the pca channel. 
  * @param  bit: set the bit value of pca channel.
  * @retval None
  */
void PCA_SetHighSpeed_output(PCA_TypeDef* PCAx,\
	                           PCA_Channel channel,\
                             uint16_t value,\
                             uint8_t bit)
{
    ...
    /*Set MAT*/		
		...
		PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel0);
		PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel0);
	}
	else if(channel == PCA_Channel1)
	{
		...
		PCAx->CCAPO   |= (bit<<PCA_Channel1);
		/*Setting Output Enablation*/
		PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel1);
		PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel1);
	}
	else if(channel == PCA_Channel2)
	{
		...
		PCAx->CCAPO  |= (bit<<PCA_Channel2);
		/*Setting Output Enablation*/
		PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel2);
		PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel2);
	}
	else if(channel == PCA_Channel3)
	{
		...
		PCAx->CCAPO   |= (bit<<PCA_Channel3);
		/*Setting Output Enablation*/
		PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel3);
		PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel3);
	}	
	else 
	{
		if(channel == PCA_Channel4)
		{
		  ...
		  PCAx->CCAPM4 |=  PCA_CCIE_ENABLE;
			...
			PCAx->CCAPO   |= (bit<<PCA_Channel4);
			/*Setting Output Enablation*/
			PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel4);
			PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel4);
		}
 }
}
...
/** @brief  set pwm output
  * @param  PCAx: Set the PCA peripheral.
  * @param  channel: select the channel of pca
  * @param  valuel: set value to the CCAP0L register.
  * @param  valueh: set value to the CCAP0H register.
  * @param  NewState: new state of the ADCx peripheral. 
  *         This parameter can be: ENABLE or DISABLE.   
  * @retval None
  */
void PCA_SetPwm_output(PCA_TypeDef* PCAx,\
	                     PCA_Channel channel,\
                       uint8_t valuel,\
                       uint8_t valueh,\
                       FunctionalState NewStatus)
{
	/*Check the parameters */
	assert_param(IS_PCA_ALL_PERIPH(PCAx));
	assert_param(IS_PCA_CHANNEL(channel));
    ...
    /*Set PWM*/		
		...
		if(NewStatus == ENABLE)
		{
		  /*Setting Output Enablation*/
			PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel0);
			PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel0);
			PCAx->POCR &= ~(PCA_PWMTOGGLE_ENABLE<<PCA_Channel0);
			PCAx->POCR |= (PCA_PWMTOGGLE_ENABLE<<PCA_Channel0);
    ...
    /*Set PWM*/	
		...
		if(NewStatus == ENABLE)
		{
			...
			PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel1);
			PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel1);
			PCAx->POCR &= ~(PCA_PWMTOGGLE_ENABLE<<PCA_Channel1);
			PCAx->POCR |= (PCA_PWMTOGGLE_ENABLE<<PCA_Channel1);
		}
		else 
		{
    ...
    /*Set PWM*/		
		...
		if(NewStatus == ENABLE)
		{
			...
			PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel2);
			PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel2);
			PCAx->POCR &= ~(PCA_PWMTOGGLE_ENABLE<<PCA_Channel2);
			PCAx->POCR |= (PCA_PWMTOGGLE_ENABLE<<PCA_Channel2);
		}
		else 
		{
    ...
    /*Set PWM*/		
		...
		if(NewStatus == ENABLE)
		{
			...
			PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel3);
			PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel3);
			PCAx->POCR &= ~(PCA_PWMTOGGLE_ENABLE<<PCA_Channel3);
			PCAx->POCR |= (PCA_PWMTOGGLE_ENABLE<<PCA_Channel3);
		}
		else 
		{
	...
	else 
	{
		if(channel == PCA_Channel4)
		{
			...
			if(NewStatus == ENABLE)
			{
			  ...
			  PCAx->POCR &= ~(PCA_OUTPUT_ENABLE<<PCA_Channel4);
			  PCAx->POCR |= (PCA_OUTPUT_ENABLE<<PCA_Channel4);
				PCAx->POCR &= ~(PCA_PWMTOGGLE_ENABLE<<PCA_Channel4);
				PCAx->POCR |= (PCA_PWMTOGGLE_ENABLE<<PCA_Channel4);
			}
			else 
			{
...
/** @brief  Clear the interrupt flag
  * @param  PCAx: Set the PCA peripheral.
  * @param  PCA_TI_FLAG: pca interrupt flag
  * @retval None
  */
...
/******************* (C) COPYRIGHT 2020 CX32L003 *****END OF FILE****/
