...
/**
  * @brief  Deinitializes the LPUARTx peripheral registers to 
		...
		(LPUART_InitStruct->LPUART_Mode==LPUART_MODE3))
	{
		...
		{  
			...
			if(LPUART_InitStruct->LPUART_BaudRateTimer_Selected) 	
			{
				...
				LPUARTx->BAUDCR  &= ~LPUART_SELF_BRG;
				...
				LPTIM->LOAD = (0x01<<16) - (int16_t)baudrate;
				/*config lptimer BGLOAD register*/
				LPTIM->BGLOAD =(0x01<<16) - (int16_t)baudrate;
				/*config lptimer CR register*/
				LPTIM->CR = LPUART_LPTIM_EABLE;
				/*open lptimer*/
				LPTIM->CR |= LPUART_LPTIMRUN_EABLE;
			}
...
/**
  ...
  *         This parameter can be: ENABLE or DISABLE.
  * @param  FunctionState: Lpuart function status
  * @retval None
  */
                ...
                FunctionalState NewState)
{
	...
	if(FunctionState == LPUART_RXEN_EABLE)	
	{
		if (NewState != DISABLE) 
		{
...
/**
  ...
  * @retval None
  */
void LPUART_ReciveCmd(LPUART_TypeDef* LPUARTx,\
	                    LPUART_InitTypeDef* LPUART_InitStruct,\
                      FunctionalState NewState)
{
	/* Check the parameters */
...
FlagStatus LPUART_GetFlagStatus(LPUART_TypeDef* LPUARTx, uint16_t LPUART_Flag)
{
	...
	if ((LPUARTx->INTSR & LPUART_Flag) != (uint16_t)RESET)
	{
		bitstatus = SET;
	}
	else
...
void LPUART_ClearITFlag(LPUART_TypeDef* LPUARTx, uint16_t LPUART_Flag)
{
	...
	/*clear it falg*/
	LPUARTx->INTCLR = (uint32_t)LPUART_Flag;
}
...
/******************* (C) COPYRIGHT 2018 CX32L003 *****END OF FILE****/
