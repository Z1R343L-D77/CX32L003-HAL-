...
ErrorStatus RTC_DeInit(void)
{
  ...
  else
  {
    ...
    do
    {
      wutwfstatus = RTC->ISR & RTC_ISR_WAITF;
      wutcounter++;  
    } while((wutcounter != INITMODE_TIMEOUT) && (wutwfstatus == 0x00));
    
    if ((RTC->ISR & RTC_ISR_WAITF) == RESET)
    {
      status = ERROR;
    }
    else
    ...
    // 	RTC->CLKTRIM &= (uint32_t)0x00000000;
...
/**
  ...
  * @retval None
  */
void RTC_GetTime(RTC_TypeDef *RTCx,\
	               uint32_t RTC_Format,\
                 RTC_TimeTypeDef* RTC_TimeStruct)
{
  uint32_t tmpreg = 0;
...
/**
  ...
  * @retval None
  */
void RTC_GetDate(RTC_TypeDef *RTCx,\
	               uint32_t RTC_Format,\
                 RTC_DateTypeDef* RTC_DateStruct)
{
  uint32_t tmpreg = 0;
 																	...
 																	uint32_t Value)
{
 ...
 //   RTCx->CLKTRIM = (uint32_t)(RTC_CalibSign|Value);
    /* Exit Initialization mode */
...
//	RTCx->CLKTRIM = (uint32_t)((uint32_t)RTC_SmoothCalibPeriod |(uint32_t)RTC_SmoothCalibTrmingValue);
