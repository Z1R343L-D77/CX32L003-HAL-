...
#define GPIO_AF_TIM1_CH2_PD6          ((uint8_t)0x01)  /* TIM1 Alternate Function mapping */
...
#define IS_GPIO_AFR(GPIO_AF)     (((GPIO_AF >= 0) &&(GPIO_AF<= 7))||\
                                 (GPIO_AF== 0x0f))
...
void GPIO_PinRemapConfig(GPIO_TypeDef* GPIOx,uint8_t GPIO_AFR, uint8_t GPIO_PinSource);
void GPIO_PortDRCmd(GPIO_TypeDef* GPIOx,uint8_t GPIO_PinSource,FunctionalState NewState);
void GPIO_PinPuPdCmd(GPIO_TypeDef* GPIOx,uint8_t GPIO_PinSource,uint8_t GPIO_PUPD);
#ifdef __cplusplus
}
#endif
